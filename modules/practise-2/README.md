...
3. Сохраняет отчёт в JSON и SOAP.
4. Отправляет отчёт указанному получателю по email.
...
| `JsonReportSaver` | Реализация сохранения отчёта в JSON. |
| `SoapReportSaver` | Реализация сохранения отчёта в XML-конверте SOAP 1.2. |
| `CompositeReportSaver` | Объединяет несколько способов сохранения без изменения основного сервиса. |
...
Создайте один отчёт, объедините JSON- и SOAP-сохранители, настройте email-отправитель и передайте их сервису отчётов. После запуска ожидаются:

- JSON- и SOAP-файлы в каталоге для отчётов с датой и временем в имени;
